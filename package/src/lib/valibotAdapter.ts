...
/**
 * Create a RuneForm instance from a Valibot schema.
 * The schema must produce an object type.
 */
export function createForm<S extends BaseSchema<unknown, unknown, BaseIssue<unknown>>>(
	schema: S,
	initialData?: Partial<InferOutput<S>>
) {
	type T = InferOutput<S> extends Record<string, unknown> ? InferOutput<S> : Record<string, unknown>;
	return new RuneForm<T>(
		createValibotValidator(schema) as Validator<T>,

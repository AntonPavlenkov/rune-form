...
import { z } from 'zod';
import { createForm as createCustomForm } from './customAdapter.js';
import { RuneForm } from './RuneForm.svelte.js';
import { createForm, createZodValidator, getAllPaths } from './zodAdapter.js';
...
describe('RuneForm additional tests', () => {
	...
	it('should handle array swap operations with proper touched and error tracking', async () => {
		...
		const form = createForm(testSchema);
...
describe('Custom Validation', () => {
	it('should work with custom validation functions', async () => {
		...
		const form = createCustomForm(customValidator, {
			name: 'John',

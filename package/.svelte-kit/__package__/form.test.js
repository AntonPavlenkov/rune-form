...
describe('RuneForm additional tests', () => {
    ...
    it('should return undefined for non-existent fields', () => {
        const field = form.getField('notAField');
        field.value = 'test';
    ...
    it('should handle array swap operations with proper touched and error tracking', async () => {
        ...
        form.data.address.parkingLots.push({ name: '', lat: 40.7128, lng: -74.006 }, // Invalid: empty name
        ...
        const parkingLotErrors = Object.keys(form.errors).filter((key) => key.startsWith('address.parkingLots'));
    ...
    it('should clear errors and touched state when array items are removed', async () => {
        ...
        form.data.address.parkingLots.push({ name: 'Valid Lot 1', lat: 40.7128, lng: -74.006 }, // Valid
    ...
    it('should clean up touched state when array items are removed after mutation', async () => {
        ...
        form.data.address.parkingLots.push({ name: 'Lot 1', lat: 40.7128, lng: -74.006 }, { name: 'Lot 2', lat: 41.0, lng: -75.0 });
        // Verify initial state
        expect(form.data.address.parkingLots).toHaveLength(2);
...
describe('getAllPaths property-based', () => {
    it('returns valid paths for random object schemas', () => {
        fc.assert(fc.property(fc.dictionary(fc.string({ minLength: 1, maxLength: 8 }), fc.constant(z.string())), (fields) => {
...
it('should debug array swap operations', () => {
    const form = RuneForm.fromSchema(schema);
    // Add two items
    form.data.address.parkingLots.push({ name: 'Lot A', lat: 40.0, lng: -74.0 }, { name: 'Lot B', lat: 41.0, lng: -75.0 });
